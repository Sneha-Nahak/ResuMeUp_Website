...
const BACKEND_FETCH_URL = BACKEND_SAVE_URL
  ? BACKEND_SAVE_URL.replace("/save", "/fetch")
  : "";
    ...
    async () => {
      ...
      try {
        ...
        if (response.ok) {
          setHasPreviousData(true);
          console.log(
            `Resume data ${method === "PUT" ? "updated" : "saved"} successfully!`
          );
  ...
  return (
    <div className="resume-builder-container">
      ...
      <section>
        ...
        <div className="input-group">
          ...
          <input
            ...
            onChange={(e) =>
              setPersonalInfo({ ...personalInfo, linkedinLink: e.target.value })
            }
          />
          <input
            ...
            onChange={(e) =>
              setPersonalInfo({ ...personalInfo, githubLink: e.target.value })
            }
          />
          <textarea
        ...
        {education.map((edu, i) => (
          <div key={i} className="input-group">
            <input
              placeholder="School"
              value={edu.school}
              onChange={(e) => updateEducationField(i, "school", e.target.value)}
            />
            <input
              placeholder="Degree"
              value={edu.degree}
            ...
            />
            <button className="delete-btn" onClick={() => removeEducation(i)}>
              Delete
            </button>
          </div>
        ))}
        ...
        {experience.map((exp, i) => (
          <div key={i} className="input-group">
            ...
            <input
              ...
              value={exp.position}
              onChange={(e) => updateExperienceField(i, "position", e.target.value)}
            />
            <input
            ...
            />
            <button className="delete-btn" onClick={() => removeExperience(i)}>
              Delete
            </button>
          </div>
        ))}
        ...
        {projects.map((proj, i) => (
          <div key={i} className="input-group">
            ...
            <input
              ...
              onChange={(e) => updateProjectField(i, "githubLink", e.target.value)}
            />
            <input
              placeholder="Live Link"
              value={proj.liveLink}
              onChange={(e) => updateProjectField(i, "liveLink", e.target.value)}
            />
            <input
              placeholder="Other/Social Link (optional)"
              value={proj.otherLink}
              onChange={(e) => updateProjectField(i, "otherLink", e.target.value)}
            />
            <button className="delete-btn" onClick={() => removeProject(i)}>
              Delete
            </button>
          </div>
        ))}
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: "20px" }}>
        ...
        <button className="pdf-btn" onClick={handleSaveAndPrint}>
          Download
        </button>
      </div>
      ...
      <div id="resume-preview" className="resume-preview" ref={resumeRef}>
        ...
        <p className="contact-line">
          ...
          {personalInfo.linkedinLink &&
            (personalInfo.email || personalInfo.phone || personalInfo.githubLink) &&
            " | "}
          {personalInfo.linkedinLink && (
            ...
            {projects.map((proj, i) => (
              <li key={i}>
                ...
                {proj.otherLink && (
                  <>
                    {" | "}
                    <a href={proj.otherLink} target="_blank" rel="noreferrer">
                      <strong>Other</strong>
                    </a>
                  </>
                )}
